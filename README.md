...
![image](https://github.com/rgjohnson98/dataviz-project-template-proposal/blob/master/CS573Dashboard1Finalb.PNG)
This second image shows the dashboard with the filter active, and set to Michigan International Speedway (MIS). I set each plot to the same track for this example, but each can be set to different tracks. As we can see from this filtered dashboard, we now know where MIS is located, that only one driver got their first win there, winners tend to start the race in the top 10 (but there have been some exceptions recently), and Chevrolet and Ford have been the most successful manufacturers there. Via the tooltips, we can see details about each filtered plot, as will be demonstrated below.
